    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 py-6 pb-24">
        ...
        <div class="glass rounded-2xl p-5 mb-8">
            ...
            @forelse($pendingOrders as $order)
                <div class="flex items-center justify-between p-4 rounded-xl {{ $order->isUrgent() ? 'bg-red-500/10 border border-red-500/20 urgent-flash' : 'bg-white/80 border border-slate-100/50' }} mb-3 transition-all">
                    <div>
                        <div class="flex items-center gap-2">
                        ...
                        <div class="text-xs text-white/70 mt-1">{{ $order->client->name }} · {{ $order->created_at->diffForHumans() }}</div>
                    </div>
                    <div class="text-right rtl:text-left">
                        <div class="text-lg font-bold text-violet-600">{{ number_format($order->total, 0) }} {{ __('SAR') }}</div>
                    </div>
                </div>
            @empty
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                @foreach($availableTechnicians as $tech)
                    <div class="flex items-center gap-3 p-3 rounded-xl bg-white/80 border border-slate-100/50">
                        <div class="w-10 h-10 rounded-xl gradient-brand flex items-center justify-center text-sm font-bold">
